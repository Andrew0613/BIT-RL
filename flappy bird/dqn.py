...
def train(model, start):
    ...
    reward_his = []
    reward_episode = 0
    writer = SummaryWriter(log_dir='runs/reward_his_without_Target')
    episode = 0
    # main infinite loop
    while iteration < model.number_of_iterations:
        ...
        if iteration % 25000 == 0:
            torch.save(model, "pretrained_model_without/current_model_" + str(iteration) + ".pth")
...
def draw(reward_his,episode):
    ...
    plt.xlabel('episode')
    plt.ylabel('reward')
    plt.title('DQN without target-net')
    plt.plot(range(episode), reward_his)
    plt.savefig('dqn_without_target.jpg')
    plt.show()
def test(model):
    game_state = GameState()
...
def main(mode):
    ...
    if mode == 'test':
        ...
        if not os.path.exists('pretrained_model_without/'):
            os.mkdir('pretrained_model_without/')
